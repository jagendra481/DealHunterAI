...
class FlipkartProvider:
    ...
    def fetch_product(self, product):
        ...
        slug_title = match_slug.group(1).replace("-", " ").title() if match_slug else "Flipkart Tracked Product"
        ...
        try:
            ...
            # Price fallback
            if price <= 0:
                price = old_price if old_price > 0 else 999.0
            ...
            logger.warning(f"[Flipkart Scraper Timeout/Notice] {err} -> using fallback Product details")
            return Product(
                user_id=user_id,
                ...
                affiliate_url=aff_url,
                asin=re.search(r"/p/([a-zA-Z0-9]+)", url).group(1) if re.search(r"/p/([a-zA-Z0-9]+)", url) else "",
                current_price=old_price or 14999.0,
                previous_price=old_price or 14999.0,
                lowest_price=old_price or 14999.0,
                highest_price=old_price or 14999.0,
                source="flipkart",
                image=old_image or "",
                rating=4.2,
                reviews=10,
